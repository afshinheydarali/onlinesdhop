        ...
        async with engine.connect() as connection:
            ...
            for row in admins:
                ...
                found = (
                    (
                        await connection.execute(
                            text("SELECT telegram_id, name, admin_code, is_active, created_at FROM admins WHERE telegram_id=:id"),
                            {"id": row["telegram_id"]},
                        )
                    )
                    .mappings()
                ...
                user = (
                    (
                        await connection.execute(
                            text("SELECT id, telegram_id, username, password_hash, is_active, role FROM users WHERE telegram_id=:id"),
                            {"id": row["telegram_id"]},
                        )
                    )
                    .mappings()
            ...
            for row in orders:
                ...
                found = (
                    (
                        await connection.execute(
                            text("SELECT " + ",".join(ORDER_FIELDS) + ",created_by_id FROM orders WHERE id=:id"),
                            {"id": row["id"]},
                        )
                    )
                    .mappings()
                ...
                if found:
                    ...
                    existing_outbox = (
                        (
                            await connection.execute(
                                text("SELECT status, attempts, error_code FROM outbox WHERE order_id=:id"),
                                {"id": row["id"]},
                            )
                        )
                        .mappings()
