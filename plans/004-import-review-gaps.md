...
4. Use normal readable multiline Python and import os directly. Run the targeted module and Ruff with the integration repository's configuration: `python -m ruff check --config pyproject.toml scripts/import_sqlite.py tests/integration/test_import_sqlite.py`. Both must return exit code zero. Commit and report actual results; no other import completeness claim.
